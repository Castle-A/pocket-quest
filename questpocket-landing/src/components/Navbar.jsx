...
const flagIcons = {
  en: (
    <svg width="18" height="18" viewBox="0 0 640 480" className="rounded-sm">
      ...
      <path fill="#CE1124" d="M0 0h640v36.4H0M0 72.8h640v36.4H0M0 145.6h640v36.4H0M0 218.4h640v36.4H0M0 291.2h640v36.4H0M0 364h640v36.4H0M0 436.8h640V480H0"/>
      <path fill="#00247D" d="M0 0h292.4v218.4H0"/>
    </svg>
  ),
  fr: (
    <svg width="18" height="18" viewBox="0 0 640 480" className="rounded-sm">
      <path fill="#FFF" d="M0 0h640v480H0"/><path fill="#002395" d="M0 0h213.3v480H0"/><path fill="#ED2939" d="M426.7 0H640v480H426.7"/>
    </svg>
  ),
  de: (
    <svg width="18" height="18" viewBox="0 0 640 480" className="rounded-sm">
      <path fill="#000" d="M0 0h640v160H0"/><path fill="#DD0000" d="M0 160h640v160H0"/><path fill="#FFCC00" d="M0 320h640v160H0"/>
    </svg>
  ),
};
    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled ? 'py-3 bg-white/80 backdrop-blur-xl shadow-sm shadow-gray-200/50' : 'py-5 bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <button onClick={() => document.getElementById('countdown')?.scrollIntoView({ behavior: 'smooth' })}
              className="text-sm text-gray-500 hover:text-gray-900 transition-colors">Features</button>
            <div className="relative group">
              ...
              <div className="absolute top-full right-0 mt-2 py-1 bg-white border border-gray-100 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 min-w-[120px]">
                {langs.map(l => (
                  ...
                  >{flagIcons[l]} <span className="text-xs uppercase">{l}</span></button>
                ))}
              </div>
            </div>
            <button data-cta
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-2 text-gray-500" aria-label="Menu">
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <button onClick={() => { document.getElementById('countdown')?.scrollIntoView({ behavior: 'smooth' }); setMobileOpen(false); }}
              className="block text-gray-600 hover:text-gray-900 text-base">Features</button>
